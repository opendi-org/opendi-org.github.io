...
## Coherence Gate (Halt / Ask / Act)

A verification step at a system boundary that determines whether information crossing into a decision has been measured for alignment before action is taken. A coherence gate produces one of three outcomes: Halt (a required condition is not satisfied — do not proceed), Ask (uncertainty is measurable and reducible — acquire the missing information), or Act (all conditions are met and uncertainty is below threshold — proceed).

A coherence gate is the verification step that determines whether the information justifying that action is coherently understood by all systems involved in producing it. Without it, Action proceeds on unverified assumptions. Halt, Ask, and Act give OpenDI practitioners a vocabulary for what to do when coherence has not been verified — which right now the glossary has no language for.

A coherence gate sits between Information and Action in the OpenDI decision flow. It is the step that converts unverified information into verified input. Halt maps to a decision deferral. Ask maps to a request for clarification or additional data. Act maps to the authorized execution of a decision on verified information.

## Context Graph

A structured, append-only record of what was missing, what was asked, and what was resolved at a boundary between systems before a decision was made. A context graph is not predefined — it is constructed through measurement at the moment two systems meet. It accumulates the resolutions required to establish coherent mutual understanding, and it persists as the audit trail for pre-decision verification.

A context graph records what was uncertain, surfaced, and resolved before the decision could be made. It is a cross-systems boundary audit trail for the pre-decision verification that the glossary does not currently have a term for.

A context graph is the artifact produced when systems at a boundary measure whether they share a common understanding before proceeding.
...
The condition in which a system proceeds with full confidence on information that different systems interpret differently — and no instrument available to the decision-maker can detect the mismatch. The system does not know it is in this state. It has not failed — it has simply never measured whether the information it received means the same thing to the sender as it does to the receiver.

For example: a company scans paper invoices from both US and EU suppliers. The scanned dates are extracted into a datastore as strings. The value 04/01/2020 appears. A decision model consumes this field to calculate payment timing and cash flow projections. The model assumes all dates are MM/DD/YYYY. For US invoices, 04/01/2020 is April 1st. For EU invoices, it is January 4th — nearly three months earlier. No error is raised. The schema validates. The pipeline runs cleanly. The cash flow projection is wrong by a fiscal quarter, and the decision-maker has no way to know — because the system never measured whether "date" meant the same thing across the two sources. That is ignorance of incoherence.
